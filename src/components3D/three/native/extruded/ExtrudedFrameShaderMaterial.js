...
const ExtrudedFrameShaderMaterial = shaderMaterial(
  /*
   _____Uniforms
 */
  {
 ...
 */
  ...
  void main (){
    ...
    //   gl_FragColor = vec4(vec3(colorizator),someMalewicz);
      ...
      //___patern: gradiented circle that can be dark / light in the center => just manipulate value before minus sign...
...
//___patern gradiented grid; 
      multiGradient = round(vUv.x * 20.)/20. + 0.1;
      multiGradient *= round(vUv.y * 20.)/20. + 0.1;
    //   gl_FragColor = vec4(vec3(multiGradient),someMalewicz);

//___patern with colors
      // vec3 darkColor = vec3(0.1 + sin(uTime*0.5) ,0.11,0.47);
      // vec3 lightColor = vec3(0.15,0.82,0.81);
      vec3 darkColor = vec3(0., abs(0. + uDraggedX *3.), 0.);
      vec3 lightColor = vec3(1.,abs(1. + uDraggedX *2.),1.);
...
export { ExtrudedFrameShaderMaterial };
