...
const PolarCoordinatesShaderMaterial = shaderMaterial(
  /*
  ----------Uniforms Section---------------------------------------
  */
  {
  ...
  */
  void main (){
     ...
     float polarWholeRange = polarCoord.x / 6.2831 + .5;

     /*
     What this: "polarCoord.x / 6.2831 + .5" does ???
     It takes us from domain -PI <> PI to 0<>1
     */
     float moveMaker = uTime*0.05;
     float twistMaker = polarCoord.y;
     uv = vec2(polarCoord.x / 6.2831 + .5 + moveMaker + twistMaker , polarCoord.y) ;

     float x = uv.x * 10.;
     float m = min(fract(x), fract(1.-x));
     float c = smoothstep(0., .1, m * .3 + .2 - uv.y);
    
    gl_FragColor = vec4(vec4(c));

  }
  `
);

export { PolarCoordinatesShaderMaterial };
